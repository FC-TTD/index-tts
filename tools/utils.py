...
def loudnorm(wav_data: np.ndarray, sr: int, target_loudness: float = -23, threshold: float = 0.99) -> tuple[np.ndarray, float]:
    ...
    # 避免除零错误
    abs_wav = np.abs(wav_data)
    # 将绝对值为0的样本替换为一个很小的非零值
...
def eq(wav_data: np.ndarray, sr: int) -> np.ndarray:
    ...
    def enhance_frequency_band(audio, sample_rate, low_freq, high_freq, gain_factor):
        ...
        high = min(high_freq / nyquist, 0.99)  # 确保高频不超过奈奎斯特频率
        
        # 检查频率范围是否有效
        if low >= high or low <= 0 or high >= 1:
            # 如果频率范围无效，则不进行滤波，直接返回原始音频
            return audio
    ...
    # 根据采样率调整频段
    nyquist = 0.5 * sr
    low_freq = min(5000, nyquist * 0.6)  # 确保低频不超过奈奎斯特频率的60%
    high_freq = min(10000, nyquist * 0.95)  # 确保高频不超过奈奎斯特频率的95%
    gain_factor = 0.2
