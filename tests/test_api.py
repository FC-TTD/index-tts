...
def check_generate_audio(
    ...
    if prompt_speech and os.path.exists(prompt_speech):
        # 正确打开文件作为二进制文件对象
        files["prompt_speech"] = (
            os.path.basename(prompt_speech),
            open(prompt_speech, "rb"),
            "audio/wav",
        )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(
                f"音频长度: {duration:.2f}秒，采样率: {sample_rate}Hz, 平均响度: {loudness:.2f} LUFS"
            )
...
def main():
    ...
    parser.add_argument(
        "--text",
        default="你怎么这么傻,昨夜明明说好了你绝不会轻举妄动的",
        help="要合成的文本",
    )
    parser.add_argument("--output", default="output.wav", help="输出音频文件路径")
    parser.add_argument("--prompt-speech", help="提示语音文件路径")
    parser.add_argument(
        "--speed", type=float, default=1.0, help="语速控制参数 (duration_ratio)"
    )
    parser.add_argument(
        "--expected-duration",
        type=float,
        default=None,
        help="期望时长（秒），用于自适应重推理",
    )
    parser.add_argument(
        "--infer-mode",
        default="普通推理",
