...
Current deployment (2026-09-13): the managed worker instance serves the existing `http://index-api` API and `http://xique` UI. Their business URLs are retained; the public proxy keeps its existing authentication and HTTPS origin. Real API synthesis and native UI upload/queue/generation/download passed through those original URLs.
...
The old ttd-fastapi-utils subtree is retained for callers not yet migrated; this new entry does not import it. Legacy fusion deployment assets are retained only for explicit rollback. The old Portainer stack has relinquished its business route labels and is scaled to zero as of 2026-09-15, with container-local outputs archived. Do not use the legacy deployment path to republish those labels.

`index-hub` in the current Compose asset is an auxiliary address for validation before cutover. Formal adoption must take over the existing business domain and preserve its API/UI URLs so callers do not change addresses. The cutover changes Caddy's backend, not the public URL; rollback preserves that same URL. A redirect to a new `-hub` domain is not a substitute. Direct API/UI acceptance through the existing business URLs has passed; Gateway managed-mode rollout and production internal usage verification passed on 2026-09-15.

`docker/compose.hub-business.yml` prepares the original `index-api`, `xique` and `xique-tts-public-proxy` routes. It replaces the auxiliary route labels and preserves the existing public Host/HTTPS headers. It requires a Hub image with `model-entry --public-origin` support and the SDK Gradio mount-prefix fix. Runtime port 8000 remains private to the model network; proxy header trust is enabled only in this overlay for the CPU entry hop. This overlay has been deployed through the controlled handoff. The generic Hub fresh-release installer still cannot replace an existing stack; use the maintained publication/handoff path for later changes.
