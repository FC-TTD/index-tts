...
def _ensure_runtime_cache_env() -> None:
    ...
    for path in (hf_home, hf_hub_cache, transformers_cache, modelscope_cache, torch_extensions_dir, torchinductor_cache_dir):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--bf16', action='store_true', default=False, help='Use BF16 to reduce memory and speed up on CUDA')
    ...
    parser.add_argument('--use_accel', action=argparse.BooleanOptionalAction, default=False, help='Use Accelerate for multi-GPU if available (default: disabled)')
    parser.add_argument('--use_torch_compile', action=argparse.BooleanOptionalAction, default=False, help='Use torch.compile for inference (default: disabled)')
    parser.add_argument('--preload_model', action=argparse.BooleanOptionalAction, default=False, help='Preload the model at startup instead of relying on SmartModel lazy loading')
    return parser
...
def validate_model_dir(model_dir: str) -> None:
    ...
    required = ('config.yaml', 'gpt.pth', 's2mel.pth', 'codec.pth', 'multilingual_zh_ja_yue_char_del.tiktoken', 'wav2vec2bert_stats.pt', 'qwen0.6bemo4-merge')
    missing = [name for name in required if not os.path.exists(os.path.join(model_dir, name))]
    if missing:
        raise FileNotFoundError(f"模型目录 {model_dir} 缺少 IndexTTS 2.5 文件: {', '.join(missing)}")
